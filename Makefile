...
init: ##=> Install OS deps and dev tools
	$(info [INFO] Bootstrapping CI system with OS packages and dev tools...)
	@$(MAKE) _install_os_packages || { echo "ERROR: Failed to install OS packages"; exit 1; }

deploy: ##=> Deploy services
	$(info [INFO] Starting deployment of services...)
	@$(MAKE) deploy.shared-lambda-layers || { echo "ERROR: Failed to deploy shared Lambda layers"; exit 1; }
	@$(MAKE) deploy.payment || { echo "ERROR: Failed to deploy payment service"; exit 1; }
	@$(MAKE) deploy.booking || { echo "ERROR: Failed to deploy booking service"; exit 1; }
	@$(MAKE) deploy.loyalty || { echo "ERROR: Failed to deploy loyalty service"; exit 1; }
#	$(MAKE) deploy.perftest

delete: ##=> Delete services
	$(info [INFO] Starting deletion of services...)
	@$(MAKE) delete.booking || { echo "ERROR: Failed to delete booking service"; exit 1; }
	@$(MAKE) delete.payment || { echo "ERROR: Failed to delete payment service"; exit 1; }
	@$(MAKE) delete.loyalty || { echo "ERROR: Failed to delete loyalty service"; exit 1; }
	@$(MAKE) delete.shared-lambda-layers || { echo "ERROR: Failed to delete shared Lambda layers"; exit 1; }
#	$(MAKE) delete.perftest
...
export.parameter:
	$(info [+] Adding new parameter named "${NAME}")
	@aws ssm put-parameter --name "$${NAME}" --type "String" --value "$${VALUE}" --overwrite || { echo "ERROR: Failed to export parameter to SSM"; exit 1; }
...
_install_os_packages:
	$(info [*] Installing jq...)
	@npm install -g jq-cli-wrapper || { echo "ERROR: Failed to install jq-cli-wrapper"; exit 1; }
	$(info [*] Upgrading Python SAM CLI and CloudFormation linter to the latest version...)
	@python3 -m pip install --upgrade --user cfn-lint aws-sam-cli || { echo "ERROR: Failed to upgrade Python SAM CLI and CloudFormation linter"; exit 1; }
	@npm -g install aws-cdk || { echo "ERROR: Failed to install AWS CDK"; exit 1; }
